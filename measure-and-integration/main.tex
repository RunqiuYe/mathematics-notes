...
\begin{ex}[Folland 4.1]
If $\card (X) \geq 2$, there exists a topology on $X$ that is 
$T_0$ but not $T_1$.
\end{ex}

\begin{proof}
Let $x \in X$, then the topology given by $\tcal = \{\emptyset\}
\cup \{Y \subset X : x \in Y\}$ is $T_0$ but not $T_1$.
\end{proof}
