...
on $E$. In either case, 
\[
\int_E g \circ f J_f  = \int_{f(E)} g \d \hcal^n.
\]
\end{enumerate}
\end{thm}
...
\begin{enumerate}
\item For $f, g \in \leb_\mu^p$, let $f \sim g$ if 
$f = g$ a.e. Then $\sim$ is a equivalence relation.

\item Let $[f]$ be the equivalence class of $f$.
...
\]
Let $g = \lim_{m \to \infty} g_m$. This is 
...
certain $m$-dimensinoal subsets of $\R^n$ ($m \leq n$).
...
\item We say $M$ is an $m$-dimensinoal $C^k$ \textbf{manifold}
