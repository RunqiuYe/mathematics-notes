...
\begin{thm}
  ...
  \begin{enumerate}
    ...
    such that $\psi_n \to f$ pointwise \ae in $X$.
    \item There exists a measurable and separably valued
    $F : X \to Y$ such that $f = F$ \ae
...
\begin{defi}
  ...
  \begin{enumerate}
    ...
    if $f = F$ \ae with $F: X \to Y$ is measurable.
    ...
    Equivalently, $f$ is the \ae limit of simple functions.
    ...
    such that $\psi_n \to f$ \ae as $n \to \infty$.
  \end{enumerate}
\end{defi}
...
\begin{eg}
  ...
  Then, $f = g$ \ae This shows that equality almost
...
\begin{enumerate}
  ...
  \to Y$, $f$ is measurable and $f = g$ \ae, then
  $g$ is measurable.
  \item If $Y$ is a metric space with $\card Y = 2$,
  $f, g: X \to Y$, $f$ measurable, $f = g$ \ae,
...
$f = g$ \ae Pick null set $N \in \mf$ such that
...
\end{cases}
\]
and $f(x) = y$ be constant. Then $f = g$ \ae,
...
\begin{proof}
  \begin{enumerate}
    ...
    $\phi_n \to g$ pointwise \ae Pick null set
...
\begin{thm}[Pettis]
  ...
  $\phi_n \to f$ pointwise \ae as $n \to \infty$.
  Moreover, the same
...
\begin{proof}
  ...
  Therefore, $\phi_n \to f$ \ae and thus
  $f$ is $\mu$-measurable.
...
$\phi \in S(X; [0, \infty])$ such that $\phi \leq f$ \ae
...
$\phi$ such that $\phi \leq f$ \ae Taking the sup
gives
\[
...
\begin{enumerate}
  ...
  \item If $f \leq g$ \ae then $\int_X f \d \mu \leq
  \int_X g \d \mu$.

  \item $\abs{\int_X f \d \mu} \leq \int_X \abs{f} \d \mu$.

  \item If $\abs{f} \leq g$ \ae and $g$ integrable, then
  $f$ is integrable.
\end{enumerate}
\end{thm}
...
\begin{enumerate}
  ...
  pick $F = f$ \ae for $F$ measurable and integrable and
...
is integrable. Further suppose $g \leq f$ \ae Then, $f$
...
\item Suppose that $\seqinfk{f_k}$ is almost everywhere
nonincreasing, that is, $f_k \geq f_{k+1}$ \ae Suppose
also that there exists an integrable function
$g : X \to \bar{\R}$ such that $g \geq f_k$ \ae for all $k \in \N$.
Then, $f$ and $f_k$ are extended integrable for all $k \in \N$,
and
\[
\lim_{k \to \infty} \int_X f_k \d \mu = \int_X f \d \mu.
...
\begin{enumerate}
  ...
  be $\mu$-measurable for all $k \in \N$ and $f_k \geq 0$ \ae
  Then,
  \[
...
\begin{enumerate}
  ...
  $\sumk^m f_k \leq \sumk^{m+1} f_k$ \ae Apply monotone
...
$g \leq f_k$ \ae for all $k \in \N$. Then the following
holds:
\begin{enumerate}
...
\]
which is
a $\mu$-measurable function. Then, $g \leq f_k$ \ae implies
$g \leq f$ \ae as well. It follows that $-f_k \leq -g$
...
$f_k \to f$ \ae and $g_k \to g$ \ae Suppose further that
$g_k$ is integrable and $\abs{f_k} \leq g_k$ \ae
...
\begin{proof}
We know $\abs{f_k} \leq g_k$ \ae, $g_k \to g$ \ae,
and $f_k \to f$ \ae Then, $\abs{f} \leq g$ \ae,
...
$f_n \to f$ \ae and $g_n \to g$ \ae
Also, suppose $g_n, g$ are integrable.
Finally suppose
$\norm{f_n} \leq g_n$ \ae and
\[
...
$\phi_n \to f$ \ae and $\int_X \norm{f - \phi_n} \d \mu \to 0$.
Then we have $T \circ \phi_n \to T \circ f$ \ae and
\[
...
is $\lambda$-integrable. Then for $\hcal^n$-\ae $y \in \R^m$,
...
$\norm{f}_p = 0$ if and only if $f = 0$ \ae
\end{cor}
...
S(X; V)$ such that $\norm{\psi_n} \leq 2 \norm{f}$
on $X$ and $\psi_n \to f$ \ae We then have
$\norm{\psi_n}^p \leq 2^p \norm{f}$, so $\psi_n
...
where $f \sim g$ if $f = g$ \ae Once we show
...
$f = g$ \ae Then $\sim$ is a equivalence relation.
...
$g(x) \neq \infty$ \ae and by monotone convergence theorem,
we have
\[
...
\end{cases}
\]
Then $\norm{F - \sumk^m f_k}_V^p \leq g^p$ \ae
and $\lim_{m \to \infty} \norm{F - \sumk^m f_k} = 0$ \ae
